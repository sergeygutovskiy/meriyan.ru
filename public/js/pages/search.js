...
var active_tags = [];
var active_seasons = [];
var active_complexities = [];
var min_price = null;
var max_price = null;
var is_discount = true;
var search_form = document.getElementById('search-form');
var tours = document.getElementById('tours').childNodes[1];
var loading_container = document.getElementById('loading');
var tours_container = document.getElementById('tours');
var input_tags = document.getElementById('input_tags');
var input_discount = document.getElementById('input-discount');
var input_seasons = document.getElementById('input-seasons');
var input_complexities = document.getElementById('input-complexities');
var input_price_range = document.getElementById('input-price');
var tags = input_tags.getElementsByTagName('button');
var seasons = input_seasons.getElementsByTagName('button');
var complexities = input_complexities.getElementsByTagName('button');
...
try {
  for (_iterator.s(); !(_step = _iterator.n()).done;) {
    ...
    tag.addEventListener('click', function (event) {
      var id = this.getAttribute('data-tag-id');

      if (active_tags.indexOf(id) === -1) {
        active_tags.push(id);
        this.classList.add('active');
      } else {
        active_tags = active_tags.filter(function (value) {
          return value !== id;
        });
...
var _iterator2 = _createForOfIteratorHelper(complexities),
    _step2;

try {
  for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
    var complexity = _step2.value;
    complexity.addEventListener('click', function (event) {
      var id = this.getAttribute('data-complexity-id');

      if (active_complexities.indexOf(id) === -1) {
        active_complexities.push(id);
        this.classList.add('active');
      } else {
        active_complexities = active_complexities.filter(function (value) {
          return value !== id;
        });
        this.classList.remove('active');
      }
...
var _iterator3 = _createForOfIteratorHelper(seasons),
    _step3;

try {
  for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
    var season = _step3.value;
    season.addEventListener('click', function (event) {
      var id = this.getAttribute('data-season-id');

      if (active_seasons.indexOf(id) === -1) {
        active_seasons.push(id);
        this.classList.add('active');
      } else {
        active_seasons = active_seasons.filter(function (value) {
          return value !== id;
        });
        this.classList.remove('active');
      }
...
input_price_range.getElementsByTagName('input')[0].addEventListener('input', function (event) {
  min_price = this.value;
});
input_price_range.getElementsByTagName('input')[1].addEventListener('input', function (event) {
...
search_form.addEventListener('submit', function (event) {
  ...
  axios__WEBPACK_IMPORTED_MODULE_0___default().post('/api/v1/tours/search', {
    min_price: min_price,
    max_price: max_price,
    is_discount: is_discount,
    ...
    var searched_tours = response.data;
    tours.innerHTML = '';
    console.log(searched_tours);
    searched_tours.forEach(function (t) {
...
function create_tour_element(tour) {
  ...
  for (var i = 0; i < tour.tags.length && i < 3; i++) {
    ...
    tour_el += i != 0 ? 'ml-3"' : '"';
    tour_el += "href=\"/tags/".concat(tour.tags[i].id, "\">");
    tour_el += "<img src=\"".concat(tour.tags[i].image_path, "\" ");
    tour_el += "title=\"".concat(tour.tags[i].name, "\"></a>");
  }

  tour_el += '</div>';
  tour_el += '<div class="m-tour-card__text"><h3 class="m-tour-card__title mb-1">';
  tour_el += "<a href=\"/tours/".concat(tour.id, "\">").concat(tour.title, "</a></h3>");
  tour_el += "<p class=\"m-tour-card__paragraph\">".concat(tour.description, "</p></div>");
  ...
  tour_el += "href=\"/tours/".concat(tour.id, "\">\u041F\u043E\u0434\u0440\u043E\u0431\u043D\u0435\u0435</a>");
  ...
  var doc = parser.parseFromString(wrapper_start + tour_el + wrapper_end, 'text/html');
